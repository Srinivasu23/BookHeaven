...
app.post('/addbook', async (req, res) => {
  ...
  try {
    ...
    const book = new Book({ title, author, description, pdf });
    await book.save();
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    if(!containsUppercase || !containsLowercase || !containsNumber ||!containsSpecial) {
      return res.status(400).json({error :"Password should contain atleast one upper case letter,lowercase letter , number and a special character"});
    }
